...
*/
static ssize_t acpi_proc_write(struct file *fd, const char *buff, size_t len, loff_t *off) {
...
*/
static int acpi_proc_read(struct file *fd, char *buf, size_t c, loff_t *off) {
   int len = 0;
...
static int __init init_acpi_call(void) {
    struct proc_dir_entry *acpi_entry 
       = proc_create("call", 438, acpi_root_dir, &fops);
